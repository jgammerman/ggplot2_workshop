...
# if you want to take something from the data you put it inside aes(), but if you just want to set something e.g. size = X or colour = Y then you set it outside of aes()
```
...
```{r}
ggplot(mpg_counted) + 
  geom_col(aes(x = class, y = count))   # geom_col() = geom_bar() with the stat set to 'identity'
```
...
We can take control by adding one explicitly. All scales follow the same naming 
conventions.
...
Positional mappings (x and y) also have associated scales (as opposed to colour scaled above).

```{r}
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  scale_x_continuous(breaks = c(3, 5, 6)) + 
  scale_y_continuous(trans = 'log10')   # trans = tranformation

# ggplot(mpg) + 
#   geom_point(aes(x = displ, y = hwy)) + 
#   scale_x_continuous(breaks = c(3, 5, 6)) + 
#   scale_y_continuous()  # no log transformation
```
...
ggplot(mpg) + 
  ...
  scale_colour_brewer(type = 'qual') + 
  scale_size(breaks = c(4, 5, 6, 8))   # scale_radius is another option, but remember that area = pi*r^2 and the eye is drawn to area!

# With scale_size_area():
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = class, size = cyl)) + 
  scale_colour_brewer(type = 'qual') + 
  scale_size_area(breaks = c(4, 5, 6, 8), max_size = 10)   # scale_radius is another option, but remember that area = pi*r^2 and the eye is drawn to area!
```
...
# Get 2 different aesthetics mapped to the same variable! And shown with the same guide - this is called guide merging. But only if they're both the same variable, like here
ggplot(mpg) + 
...
# See previous chunk for solution
```
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_wrap(~ drv, scales = "free")  # both axes are free


ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_wrap(~ drv, scales = "free_y")  # only y-axis is free
...
# Make the space of each plot look better:
ggplot(mpg) + 
  geom_bar(aes(y = manufacturer)) + 
...
ggplot(mpg) + 
  geom_bar(aes(x = class)) + 
  scale_y_continuous(limits = c(0, 40))  # this removs data outside the limits 
```
...
ggplot(mpg) + 
  geom_bar(aes(x = class)) + 
  coord_cartesian(ylim = c(0, 40))  # ALWAYS ZOOM USING COORD RATHER THAN SCALE!!
```
...
ggplot(mpg) + 
  ...
  theme(
    ...
    strip.text = element_text(face = 'bold', hjust = 0),  # makes things bold and left-justified
    plot.caption = element_text(face = 'italic'),  # makes caption italic
    panel.grid.major = element_line('white', size = 0.5),  # panel grid is white
